...
```
pack build my-python-func --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.0.10
```

Where `my-python-func` is the name of your runnable function image, later used by Docker.
...
```
docker run -it --rm -p 8080:8080 my-python-func
```
...
```
curl -i -X POST -H "Content-Type: application/cloudevents+json" -d @cloudevent.json http://localhost:8080
```
...
```
tanzu apps workload create my-python-func -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/my-python-func-source \
  --type web
```
...
```
tanzu apps workload get my-python-func
```
